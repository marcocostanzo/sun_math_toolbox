...
void UnitQuaternion::setV( const TooN::Vector<3>& v ) {}
    ...
    UnitQuaternion::UnitQuaternion( const Vector<4>& q_vec ){
        ...
        if( fabs(n-1.0) > 10.0*GEOMETRY_HELPER_EPSILON ){
            cout << PORTING_FUNCTIONS_WARN_COLOR 
                    "[UnitQuaternion] UnitQuaternion( const Vector<4>& q_vec ): input Vector is not Unit! i will do an automatic normalization..." 
    ...
    UnitQuaternion::UnitQuaternion( double s, const Vector<3>& v ):
    ...
    UnitQuaternion::UnitQuaternion( const Quaternion& q ){
        ...
        if( fabs(n - 1.0) > 10.0*GEOMETRY_HELPER_EPSILON ){
            cout << PORTING_FUNCTIONS_WARN_COLOR 
                    "[UnitQuaternion] WARN in UnitQuaternion( const Quaternion& q ): input Quaternion is not Unit! i will do an automatic normalization... " << 
                    "Maybe you want to use UnitQuaternion unit(const Quaternion&)"
                    PORTING_FUNCTIONS_CRESET << endl;
        }
    ...
    */
    ...
    Vector<3> UnitQuaternion::mtimes( const Vector<3>& v ) const{
        Quaternion q = (Quaternion::mtimes( Quaternion(0.0,v) ) ) * ::inv(*this);
        return q.getV();
    }
    ...
    /*======TYPE CONVERSION METHODS======*/
        ...
        */
        bool UnitQuaternion::eq(const UnitQuaternion& q2) const{
            Vector<4> q1_vec = getDouble();
            Vector<4> q2_vec = q2.getDouble();
            return ( sum( abs(q1_vec - q2_vec) ) < 100.0*GEOMETRY_HELPER_EPSILON ) || ( sum( abs(q1_vec + q2_vec) ) < 100.0*GEOMETRY_HELPER_EPSILON );
        }
        ...
        */  
        Matrix<3,3> UnitQuaternion::torot() const{
            ...
            return Data(
                ...
                1.0-2.0*( pow(y,2)+pow(z,2) ),               2.0*( x*y-s*z ),               2.0*( x*z+s*y ),
                              2.0*( x*y+s*z ), 1.0-2.0*( pow(x,2)+pow(z,2) ),               2.0*( y*z-s*x ),
                              2.0*( x*z-s*y ),               2.0*( y*z+s*x ), 1.0-2.0*( pow(x,2)+pow(y,2) )
